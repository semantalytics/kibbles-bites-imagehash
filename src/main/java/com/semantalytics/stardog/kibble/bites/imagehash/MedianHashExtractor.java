...
public class MedianHashExtractor implements RDFExtractor {

    private static final int BIT_RESOLUTION = 32;

    private final HashingAlgorithm hasher = new MedianHash(BIT_RESOLUTION);
    ...
    public StatementSource extract(Connection theConnection, IRI theDocIri, Path theDocContents) throws BitesException {
        try {
            if (theDocContents.toFile().length() > 0L) {
                ...
                aModel.add(Values.statement(theDocIri, ImagehashVocabulary.medianHash.iri, Values.literal(imageHash.getHashValue())));
