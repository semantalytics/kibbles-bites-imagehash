...
public class DifferenceHashExtractor implements RDFExtractor {

    private static final int BIT_RESOLUTION = 32;

    private final HashingAlgorithm hasher = new DifferenceHash(BIT_RESOLUTION, DifferenceHash.Precision.Double);
    ...
    public StatementSource extract(Connection theConnection, IRI theDocIri, Path theDocContents) throws BitesException {
        try {
            if (theDocContents.toFile().length() > 0L) {
                ...
                aModel.add(Values.statement(theDocIri, ImagehashVocabulary.differenceHash.iri, Values.literal(imageHash.getHashValue())));
