...
public class PerceptiveHashExtractor implements RDFExtractor {

    private static final int BIT_RESOLUTION = 32;

    private final HashingAlgorithm hasher = new PerceptiveHash(BIT_RESOLUTION);
    ...
    public StatementSource extract(Connection theConnection, IRI theDocIri, Path theDocContents) throws BitesException {
        try {
            if (theDocContents.toFile().length() > 0L) {
                ...
                aModel.add(Values.statement(theDocIri, ImagehashVocabulary.perceptiveHash.iri, Values.literal(imageHash.getHashValue())));
