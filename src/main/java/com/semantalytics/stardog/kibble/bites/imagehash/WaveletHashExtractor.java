...
public class WaveletHashExtractor implements RDFExtractor {

    private static final int BIT_RESOLUTION = 32;
    private static final int CYCLES = 3;

    private final HashingAlgorithm hasher = new WaveletHash(BIT_RESOLUTION, CYCLES);
    ...
    public StatementSource extract(Connection theConnection, IRI theDocIri, Path theDocContents) throws BitesException {
        try {
            if (theDocContents.toFile().length() > 0L) {
                ...
                aModel.add(Values.statement(theDocIri, ImagehashVocabulary.waveletHash.iri, Values.literal(imageHash.getHashValue())));
